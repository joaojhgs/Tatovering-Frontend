...
function Tatuadores({ id }: { id: string }) {
  ...
  const getTatuagens = () => {
    axios
      .get(
        `${process.env.NEXT_PUBLIC_API_URL}/tatuagens/tatuador/${id}`,
      )
      .then((e) => {
    ...
    <div className="h-screen w-screen bg-white">
      <div className="flex h-1/3 items-end justify-end bg-gray-500 relative">
        <Image
          ...
          src={(tatuador?.imagem_capa.length && tatuador?.imagem_capa.length > 0) ? tatuador?.imagem_capa : img}
          alt="imagem"
          fill
        />
      ...
      <div className="flex w-screen flex-row px-12">
        <div className="h-1/2 w-1/3 ">
          <div className="absolute left-20 -mt-24 flex flex-col items-center justify-center gap-10 z-40 bg-transparent ">
            <Image
              className="h-52 w-52 rounded-full object-cover "
              src={(tatuador?.imagem_perfil.length && tatuador?.imagem_perfil.length > 0) ? tatuador?.imagem_perfil : img}
              alt="imagem"
              height={208}
              width={208}
        ...
        <div className=" flex w-1/2 flex-col items-center gap-5 pt-10">
          {tatuagens && tatuagens?.length > 0 ?
            <div className="mx-auto my-2">
              ...
              />
            </div> : <Empty description="Nenhuma tatuagem" />}
        </div>
